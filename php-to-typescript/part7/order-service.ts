// Part VII — Building a real app
// A slice of the Hono port from Ch 25 — service layer without HTTP plumbing.
...
export type OrderError = { kind: "invalid"; details: string } | { kind: "not-found"; id: string };
...
export function createOrderService(deps: {
  ...
  return {
    ...
    async fetch(id: string) {
      ...
      if (order === null) return { ok: false, error: { kind: "not-found", id } };
      return { ok: true, value: order };
    },
  };
}
